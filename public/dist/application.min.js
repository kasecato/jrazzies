...
var ApplicationConfiguration = function () {
    var applicationModuleName = "j-razzies", applicationModuleVendorDependencies = ["ngResource", "ngCookies", "ngAnimate", "ngTouch", "ngSanitize", "ui.router", "ui.bootstrap", "ui.utils"], registerModule = function (moduleName, dependencies) {
...
}), ApplicationConfiguration.registerModule("cinemas"), angular.module("cinemas").controller("RatingCtrl", ["$scope", function ($scope) {
    $scope.rate = 1, $scope.max = 10, $scope.isReadonly = !1, $scope.hoveringOver = function (value) {
        $scope.overStar = value, $scope.percent = 10 * (value / $scope.max)
    }, $scope.ratingStates = [{
...
}]), ApplicationConfiguration.registerModule("core"), ApplicationConfiguration.registerModule("users"), angular.module("cinemas").run(["Menus", function (Menus) {
    Menus.addMenuItem("topbar", "Cinemas", "cinemas", "dropdown", "/cinemas(/create)?"), Menus.addSubMenuItem("topbar", "cinemas", "List Cinemas", "cinemas"), Menus.addSubMenuItem("topbar", "cinemas", "New Cinema", "cinemas/create")
}]), angular.module("cinemas").config(["$stateProvider", function ($stateProvider) {
    $stateProvider.state("listCinemas", {
        url: "/cinemas",
...
}]), angular.module("cinemas").controller("CinemasController", ["$scope", "$stateParams", "$location", "Authentication", "Cinemas", function ($scope, $stateParams, $location, Authentication, Cinemas) {
    $scope.authentication = Authentication, $scope.create = function () {
        var cinema = new Cinemas({name: this.name, rate: this.$$childHead.rate, comment: this.comment});
        cinema.$save(function (response) {
            $location.path("cinemas/" + response._id), $scope.name = "", $scope.rate = 1, $scope.comment = ""
        ...
        if (cinema) {
            cinema.$remove();
            for (var i in $scope.cinemas)$scope.cinemas[i] === cinema && $scope.cinemas.splice(i, 1)
        } else $scope.cinema.$remove(function () {
        ...
        $scope.cinema = Cinemas.get({cinemaId: $stateParams.cinemaId})
    }
}]), angular.module("cinemas").factory("Cinemas", ["$resource", function ($resource) {
    return $resource("cinemas/:cinemaId", {cinemaId: "@_id"}, {update: {method: "PUT"}})
